      MODULE bundle1  
        ...
        !| |    DELETE BUNDLE ELEMENT  : delete_b1(set, eps)                                  | |
        !| |    DELETE BUNDLE ELEMENT  : delete_b1_ind(set, ind)                              | |
        !| |                                                                                  | |
        !| |                                                                                  | |
        ...
        !**************************************************************************************   
           ...
           SUBROUTINE delete_b1_ind(set, ind)
               !
               ! Deletes from the bundle 'set' elements with index 'ind'.
               !
               ! NOTICE: the linearization error for the' current element' is always zero so it is never deleted/removed.             
               !
               IMPLICIT NONE
               ...
               INTEGER :: j, up
                ...
                IF (ind>0) THEN
                    ...
                    DO j = ind+1, up
                      set%b_elements(j-1)%subgrad = set%b_elements(j)%subgrad    
                      set%b_elements(j-1)%lin_error = set%b_elements(j)%lin_error
                    END DO
                  
                    set%b_size = up - 1                       ! the new number of stored subgradients
                    set%indeksi = up                          ! the place where the next bundle element will be added
                    IF (set%b_size /= set%b_maxsize) THEN     ! we test whether the bundle is full or not
                        set%full = .FALSE.
                    ELSE
                        set%full = .TRUE.
        ...
        !************************************************************************************** 
           ...
           SUBROUTINE init_bundle_b1(set, set_size, grad_length) 
               ...
               !**************************** NEEDED FROM USER *************************************
               TYPE(kimppu1), INTENT(INOUT) :: set          ! bundle
               INTEGER, INTENT(IN):: set_size, grad_length  ! bundle size and the length of subgradients
               !**************************** OTHER VARIABLES **************************************
               INTEGER :: i, allocstat
